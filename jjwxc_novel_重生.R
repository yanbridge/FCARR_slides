...
for (i in 1:590){
  web=read_html(str_c("http://www.jjwxc.net/bookbase.php?fw0=0&fbsj=0&ycx0=0&xx0=0&mainview0=0&sd0=0&lx0=0&fg0=0&bq75=75&sortType=0&isfinish=0&collectiontypes=ors&searchkeywords=&page=",i),encoding = "GBK")
  #web<- read_html(str_c("http://www.jjwxc.net/fenzhan/yq/bgg.html",i),encoding="utf-8")
  #用SelectorGadget定位节点信息并爬取,同时进行数据清洗
  
  writer_name<-web%>%html_nodes("td:nth-child(1) a")%>%html_text()
  writer_name=gsub("\n","",writer_name)
  writer_name=gsub(" ","",writer_name)
  #writer_name=writer_name[1:100]
...
label=rep("重生",dim(writer_final)[1])
writer_final=cbind(writer_final,label)
write.csv(writer_final,file="C:\\Users\\Administrator\\Desktop\\实习\\jjwxc_novel_重生.csv")
